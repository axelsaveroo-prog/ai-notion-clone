...
export function BottomNav() {
  ...
  return (
    <div className="fixed bottom-6 left-0 right-0 z-50 flex justify-center px-4 pointer-events-none">
      <nav className={`pointer-events-auto bg-[#141416]/95 backdrop-blur-xl border border-[#222226] rounded-full shadow-2xl flex items-center transition-all duration-300 ease-out ${
        isAiOpen ? "px-5 py-3 w-full max-w-sm sm:max-w-md" : "px-4 py-2.5 gap-3 sm:gap-4"
      }`}>
        {isAiOpen ? (
          <form onSubmit={(e) => { e.preventDefault(); setPrompt(""); setIsAiOpen(false); }} className="flex items-center gap-3 w-full">
            ...
            <input
              ...
              className="w-full bg-transparent text-sm text-white placeholder:text-gray-500 focus:outline-none px-1 py-1"
            />
